...
fn update_runnable(
    ...
    next_is_current_ctxt: bool,
) -> UpdateResult {
    // Ignore contexts that are already running - on other threads. If the scheduler would normally
    // have chosen that context A switch to Runnable context B, but context A had already
    // direct-switched to B, then B should still be switched to.
    if context.running && !next_is_current_ctxt {
...
fn schedule(
    ...
    *percpu.switch_internals.sched_ctxt.borrow_mut() = Some(Arc::clone(
    ...
    next_sched_context.schedule_time = schedule_time;
...
fn select_next_context(
        ...
        contexts_data.queue.iter()
    {
        ...
        let next_is_current_ctxt = context::is_current(&context_lock);
        ...
        let sw = update_runnable(
            &mut guard,
            cpu_id,
            schedule_time,
            next_is_current_sched,
            next_is_current_ctxt,
        );
...
fn try_direct_switch_back(
    ...
    // False by the above ptr_eq check
    let next_is_current_context = false;
...
pub fn direct_switch_to(
    ...
    // Always false by the above ptr_eq check.
    let next_is_current_ctxt = false;
...
unsafe fn switch_to(
    ...
    // thus check if it has been removed, and if so reinsert it to the fast wakeup queue.
    {
        ...
        {
            //info!("Switching `{}` => `{}`, prev was not in any runqueue despite being runnable", prev_context_guard.name, next_context_guard.name);
            percpu
                .switch_internals
                .local_wakeup_list
                .borrow_mut()
                .push(WeakContextRef(Arc::downgrade(ArcRwLockWriteGuard::rwlock(
                    &prev_context_guard,
                ))));
        }
    }
