...
pub fn detect_extended_topology() -> Result<TopologyMap, &'static str> {
    ...
    let max_leaf = cpuid.get_vendor_info()
    ...
    let is_hybrid = cpuid.get_extended_feature_info()
        .map(|features| features.has_hybrid())
        .unwrap_or(false);
    
    log::info!("CPU topology detection: leaf={:#x}, hybrid={}", topology_leaf, is_hybrid);
    ...
    let current_apic_id = unsafe {
        crate::arch::device::local_apic::LOCAL_APIC.read().id()
    };
