...
unsafe fn map_phys_contiguous_inactive<A: Arch>(
    mapper: &mut PageMapper<A, &mut BumpAllocator<A>>,
    ...
    // can be mapped with. This of course doesn't mean they are aligned at that level, just that
    // `phys` can be mapped to `virt` at `highest_map_level` if such a window fits (or a larger
    // "leaky" window, but TODO that would need support for remapping say a 4k page region within a
    // 2M huge page).
    let highest_map_level: u8 = ((phys_base.data() ^ virt_base.data()).trailing_zeros()
        - A::PAGE_SHIFT as u32)
        .div_floor(A::PAGE_ENTRY_SHIFT as u32)
        .min(u32::from(highest_supported_level))
        .try_into()
        .expect("unrealistic page table depth");

    debug!(
        "Contiguously mapping {:#0x} => {:#0x}, {page_count} pages, level {highest_map_level}",
        phys_base.data(),
        virt_base.data()
    );
    ...
    for level in 0..=highest_map_level {
        ...
        debug!(
            "head+middle lvl {level} next {next_level} pages_per_iter {pages_this_level} base {:#0x} remaining {page_count}",
            phys_base.data()
        );
    ...
    for level in (0..highest_map_level).rev() {
        let pages_this_level = 1 << (A::PAGE_ENTRY_SHIFT * usize::from(level));
        debug!(
            "tail lvl {level} pages {pages_this_level} base {:#0x} remaining {page_count}",
            phys_base.data()
        );
