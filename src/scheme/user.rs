...
impl UserInner {
    ...
    fn capture_inner<const READ: bool, const WRITE: bool>(
        ...
        let mut fake_token = unsafe { CleanLockToken::new() };

        let head_buf_opt = if !head_part_of_buf.is_empty() {
            // FIXME: Signal context can probably recursively use head/tail.
            let mut array = BorrowedHtBuf::head_locked(fake_token.downgrade())?;
            if READ {
        ...
        let tail_buf_opt = if !tail_part_of_buf.is_empty() {
            // FIXME: Signal context can probably recursively use head/tail.
            let mut array = BorrowedHtBuf::tail_locked(fake_token.downgrade())?;
        ...
        if let Some(middle_page_count_nz) = NonZeroUsize::new(middle_page_count) {
            dst_space.mmap(
                &dst_space_lock,
                Some(first_middle_dst_page),
                middle_page_count_nz,
                map_flags | MAP_FIXED_NOREPLACE,
                None,
